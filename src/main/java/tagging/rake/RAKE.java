...
public class RAKE {
	public static Collection<String> rake(String text, int n, Collection<String> stoplist, Collection<String> phraseDelimiters, Collection<String> wordDelimiters) {
		...
		candidateKeywords = getTopCandidateKeywords(ckScores, 9);
		
		Collection<String> keywords = new ArrayList<String>(n);
		for (CandidateKeyword ck : candidateKeywords) keywords.add(ck.toString());
		
		return keywords;
	}
	
	public static Collection<String> rawe(String text, int n, Collection<String> stoplist, Collection<String> phraseDelimiters, Collection<String> wordDelimiters) {
		Collection<String> tokens = toTokens(text, wordDelimiters);
		Collection<CandidateKeyword> candidateKeywords = toCandidateKeywords(tokens, stoplist, phraseDelimiters);
		CoOccurrenceMatrix com = new CoOccurrenceMatrix(candidateKeywords);
		Map<String, Double> scores = degFreq(com);
		
		return getTopWords(scores, n);
	}
	
	
	public static Collection<String> getPhraseDelimiters() {
	...
	public static Collection<String> getStopList() {
	...
	public static Collection<String> getWordDelimiters() {
		LinkedList<String> delimiters = new LinkedList<String>();
		delimiters.add("?");
		delimiters.add("!");
		delimiters.add("\n");
	...
	public static Collection<CandidateKeyword> getTopCandidateKeywords(Map<CandidateKeyword, Double> ckScores, int n) {
		...
		SortedSet<Entry<CandidateKeyword, Double>> ss = new TreeSet<Entry<CandidateKeyword,Double>>(ckScoreComparator);
	...
	public static Collection<String> getTopWords(Map<String, Double> scores, int n) {
		if(n >= scores.size()) return scores.keySet();
		
		SortedSet<Entry<String, Double>> ss = new TreeSet<Entry<String,Double>>(wordScoreComparator);
		
		ss.addAll(scores.entrySet());
		
		ArrayList<String> topKeywords = new ArrayList<String>(n);
		int i = ss.size() - n;
		int j = 0;
		for (Entry<String, Double> entry : ss)
			if(j++ >= i) topKeywords.add(entry.getKey());
		
		return topKeywords;
	}
	
	private static final Comparator<Entry<CandidateKeyword, Double>> ckScoreComparator = new Comparator<Entry<CandidateKeyword, Double>>() {
		@Override
	...
	private static final Comparator<Entry<String, Double>> wordScoreComparator = new Comparator<Entry<String, Double>>() {
		@Override
