		...
		for (Integer n : this.trials.keySet()) {
			for (ExtendedTrial trial : this.trials.get(n)) {
				...
				for (Classification classification : trial) {
					...
					pw.write(trainLabeledInstances.labelSize(classIdx) + ", ");
					pw.write(testLabeledInstances.labelSize(classIdx) + ", ");
					...
					for(int i=0; i < nl; ++i) {
						...
						if(i==0) break; // TODO: for testing.. I don't need so much data
						if(i+1 < nl) pw.write(", ");
					}
					pw.write('\n');
				}
