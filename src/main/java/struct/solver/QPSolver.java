...
public class QPSolver {
	private static Logger logger = Logger.getLogger(QPSolver.class.getName());
	...
	public double[] solve(QPConstraints constraints) {
		...
		default:
			logger.warning("No such qp type : " + qp_params.qpsolver_type);
		}		
		return null;		
	}
	...
	public QPOutput solve_full(QPConstraints constraints) {
		...
		default:
			logger.warning("No such full qp type : " + qp_params.qpsolver_type);
		}		
		return null;		
	}	
	...
	public double[] hildreth_sequential_fast(QPConstraints constraints) {
		...
		int i;
		SLFeatureVector[] a = constraints.a;
		...
		for(i = 0; i < K; i++) {
			nrm[i] = SLFeatureVector.dotProduct(a[i],a[i]);
		}
		...
		while (cont) {
			...
			for (int ind=0; ind<K; ind++) {
				...
				if (kkt > qp_params.zero) {
					...
					if ( Math.abs(diff_alpha) > qp_params.zero ) {
						for (SLFeatureVector curr = a[ind]; curr != null; curr = curr.next) {
			...
			logger.info("max_kkt_i = " + max_kkt_i + " max_kkt = " + max_kkt + " iter = " + iter + " no_skip_constraints = " + no_skip_constraints);
			timer[max_kkt_i]=1;
			iter++;
			...
			if  (iter >= qp_params.max_iter) 
				...
				if (!is_timed) 
					...
					logger.info("QPSolver : readjusting timer_adjust from " + timer_adjust);
					timer_adjust /= qp_params.timer_div;
					for (int p=0; p<K; p++)
						timer[p]=0;
					logger.info("QPSolver : readjusted timer_adjust to " + timer_adjust);
		...
		if (iter >= qp_params.max_iter)
			logger.info("QPSolver : iter >= MAX_ITER");
		return alpha;
	}
	...
	public double[] hildreth_sequential_alg_b0(QPConstraints cns) {
		...
		int i;
		logger.info("In hildreth sequential alg b0");
		logger.info(cns.toString());
		...
		for(i = 0; i < K; i++) {
			nrm1[i] = SLFeatureVector.dotProduct(cns.a1[i],cns.a1[i]);
			nrm2[i] = SLFeatureVector.dotProduct(cns.a2[i],cns.a2[i]);
		}
		...
		while (cont) {
			...
			for (int ind=0; ind<K; ind++) {
				...
				if (kkt > qp_params.zero) {
					...
					if ( Math.abs(diff_alpha) > qp_params.zero ) {
						for (SLFeatureVector curr = cns.a1[ind]; curr != null; curr = curr.next) {
							if (curr.index < 0) continue;
							w1[curr.index] += diff_alpha * curr.value;
						}
						for (SLFeatureVector curr = cns.a2[ind]; curr != null; curr = curr.next) {
							if (curr.index < 0) continue;
	...
	public QPOutput hildreth_sequential_alg_b00_full(QPConstraints cns) {
		...
		int i;
		logger.info("In hildreth sequential alg b00 full");
		logger.info(cns.toString());
		...
		for(i = 0; i < K; i++) {
			...
			if ((nrm1[i] <= 0) || (nrm1[i]==Double.POSITIVE_INFINITY))
				logger.warning("ERROR: nrm1["+i+"]=" + nrm1[i]);
			else {
				if (nrm1[i]<min_nrm1)
					min_nrm1=nrm1[i];
			}
			...
			if ((nrm2[i] <= 0) || (nrm2[i]==Double.POSITIVE_INFINITY))
				logger.warning("ERROR: nrm2["+i+"]=" + nrm2[i]);
			else {
				if (nrm2[i]<min_nrm2)
					min_nrm2=nrm2[i];
			}
		...
		while (cont) {
			...
			for (int ind=0; ind<K; ind++) {
				...
				if (kkt > qp_params.zero) {
					...
					if (Math.abs(diff_alpha) == Double.POSITIVE_INFINITY)
						logger.warning("ERROR: diff_alpha= " + diff_alpha + " ind= " + ind + " alpha[ind]=" + alpha[ind]);
					...
					if ( Math.abs(diff_alpha) > qp_params.zero ) {
						for (SLFeatureVector curr = cns.a1[ind]; curr != null; curr = curr.next) {
							...
							if (w1[curr.index] == Double.POSITIVE_INFINITY)
								logger.warning("ERROR: w1["+curr.index + "]=" + w1[curr.index] + " ind= " + ind);
						}
						for (SLFeatureVector curr = cns.a2[ind]; curr != null; curr = curr.next) {
							if (curr.index < 0) continue;
							w2[curr.index] += diff_alpha * curr.value * invnu;
							if (w2[curr.index] == Double.POSITIVE_INFINITY)
								logger.warning("ERROR: w2["+curr.index + "]=" + w2[curr.index] + " ind= " + ind);
						}
			...
			for (i=0; i<cns.dim2; i++) {
				...
				if (kkt_f == Double.POSITIVE_INFINITY) {
					logger.warning(" ERROR: kkt_f= " + kkt_f +
							" i= " + i );
				}
				if (kkt_f > max_kkt_f) {
					max_kkt_f = kkt_f;
			...
			logger.info("max_kkt_i = " + max_kkt_i + 
					" max_kkt = " + max_kkt + 
					" iter = " + iter + 
	...
	public double[] hildreth_sequential_alg_b00_blas(QPConstraints cns) {
		...
		int i;
		logger.info("In hildreth sequential alg b00 blas");
		logger.info(cns.toString());
		...
		while (cont) {
			...
			logger.info("max_kkt_i = " + max_kkt_i + " max_kkt = " + max_kkt + " iter = " + iter + " no_skip_constraints = " + no_skip_constraints + " max_kkt_f_i = " + max_kkt_f_i + " max_kkt_f = " + max_kkt_f + " iter = " + iter + " nu= " + nu + " eps = " + eps);
	...
	public double[] hildreth_sequential_alg_b(QPConstraints cns) {
		...
		int i;
		logger.info("In hildreth sequential alg b");
		logger.info(cns.toString());
		...
		while (cont) {
			...
			logger.info("max_kkt_i = " + max_kkt_i + " max_kkt = " + max_kkt + " iter = " + iter + " nu = " + nu);
	...
	public double[] hildreth_sequential_alg_a(QPConstraints cns) {
		...
		int i;
		logger.info("In hildreth sequential alg a");
		logger.info(cns.toString());
		...
		while (cont) {
			...
			for (int ind=0; ind<K; ind++) {
				...
				if (kkt > qp_params.zero) {
					...
					logger.info("alpha[ind]=" + alpha[ind] + " diff_alpha=" + diff_alpha);
					if ( Math.abs(diff_alpha) > qp_params.zero ) {
						for (SLFeatureVector curr = cns.a1[ind]; curr != null; curr = curr.next) {
							if (curr.index < 0) continue;
							w1[curr.index] += diff_alpha * curr.value;
	...
	public static double[] hildreth(SLFeatureVector[] a, double[] b) {
		...
		for(i = 0; i < K; i++) {
			A[i][i] = SLFeatureVector.dotProduct(a[i],a[i]);
			is_computed[i] = false;
		}
		...
		while (max_kkt >= EPS && iter < MAX_ITER) {
			...
			if (!is_computed[max_kkt_i]) {
				for(i = 0; i < K; i++) {
					A[i][max_kkt_i] = SLFeatureVector.dotProduct(a[i],a[max_kkt_i]); // for version 1
	...
	public static double[] hildreth(SLFeatureVector[] a, double[] b, double C) {
	...
	public static double blas_mult(SLFeatureVector fv, double[] w) {
