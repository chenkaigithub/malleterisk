...
public class KBestMiraClassifierTrainer extends ClassifierTrainer {
	 ...
	 * Trains without slack variable (C = infinity).
	 * Trains with averaging. 
	 * @param K - the K in KBestMira
	 */
	 ...
	 * Trains with averaging.
	 * @param K - the K in KBestMira
	 * @param iterations - the number of iterations to use in training
	 */
	 ...
	 * @param K - the K in KBestMira
	 * @param iterations - the number of iterations to use in training
 	 * @param C - The slack variable (clipping). Reasonable values are .1, .01. 1 usually ensures a full update, 0 will never update.
	 */
	...
	/**
	 * @param averaging - Train with averaging.
	 * @param K - the K in KBestMira
	 * @param iterations - the number of iterations to use in training
 	 * @param C - The slack variable (clipping). Reasonable values are .1, .01. 1 usually ensures a full update, 0 will never update.
	 */
	...
	/**Creates a new linear classifier and trains it or else 
	 * just trains the classifier if the classifier is not null.
	 */
	public Classifier train(InstanceList trainingSet,
			InstanceList validationSet, InstanceList testSet,
			Classifier initialClassifier) {
		
		if(trainingSet == null) throw new IllegalArgumentException("Training set cannot be null!");
		
		if(validationSet != null) {
			if(!(validationSet.getDataAlphabet() == trainingSet.getDataAlphabet() && validationSet.getTargetAlphabet()==trainingSet.getTargetAlphabet())) {
				throw new IllegalArgumentException("Validation set alphabet does not match training set alphabet, aborting!");
			}
		}
		
		if(testSet != null) {
			if(!(testSet.getDataAlphabet() == trainingSet.getDataAlphabet() && testSet.getTargetAlphabet() == trainingSet.getTargetAlphabet())) {
				throw new IllegalArgumentException("Test set alphabet does not match training set alphabet, aborting!");
			}
		}
		
		if(initialClassifier != null) {
			if(!(initialClassifier.getAlphabet() == trainingSet.getDataAlphabet() && initialClassifier.getLabelAlphabet() == trainingSet.getTargetAlphabet())) {
				throw new IllegalArgumentException("Initial classifier alphabet does not match training set alphabet, aborting!");
			}			
	 ...
	 * @throws IOException
	 */
	private void trainClassifier(LinearClassifier linearClassifier, InstanceList trainingSet, int numIter) throws IOException {
		...
		onlinelearner.train(trainingData, new KBestMiraUpdator(this.K, this.C), linearClassifier.getPredictor(), numIter);
	...
	public Classifier getClassifier() {
		if(classifier == null) 
			throw new RuntimeException("Classifier has not been initialized. Execute train() first!");
	...
	public Classifier train(InstanceList trainingSet) {
		if(trainingSet == null) throw new IllegalArgumentException("Training set cannot be null!");
		this.classifier = getNewClassifier(trainingSet); 
