...
public class SequenceEvaluator implements SLEvaluator {
	...
	public void evaluate(SLInstance[] data, Predictor predictor) {
		...
		for(int k = 0; k < data.length; k++) {
			...
			String[] pred_spans = ((SequenceLabel)prediction.getBestLabel()).tags;
			...
			for(int i=0; i < act_spans.length; i++) {
				if(act_spans[i].equals(pred_spans[i])) myCorrect++;
			}
		...
		for(int i = 0; i < correct.length; i++) {
			...
			logger.info("\tGuessed: " + guessed[i] + ", Actual: " 
					+ real[i] + ", Correct: " + correct[i]);
			double prec = (double)correct[i]/guessed[i];
			double rec = (double)correct[i]/real[i];
			double fmeas = (2*prec*rec)/(prec+rec);
			logger.info("\tPrecision: " + nf.format(prec) 
					+ ", Recall: " + nf.format(rec) 
					+ ", F-meas: " + nf.format(fmeas));
			tot_corr += correct[i];
			tot_real += real[i];
			tot_guessed += guessed[i];
		}
		logger.info("Total:");
		logger.info("\tGuessed: " + tot_guessed + ", Actual: " 
				+ tot_real + ", Correct: " + tot_corr);
