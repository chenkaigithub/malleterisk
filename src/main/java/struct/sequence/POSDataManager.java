...
public class POSDataManager extends SequenceDataManager {
		...
		ObjectOutputStream out = 
			...
			while(line != null) {
				if(num1 > 0 && num1 % 500 == 0)
					logger.info("Creating Feature Vector Instance: " + num1
	...
	/*
	 *  (non-Javadoc)
	 * @see struct.sequence.SequenceDataManager#createAlphabets(java.lang.String, boolean)
	 */
		...
		for(int i = 0; i < toks.length; i++) {
			...
			for(int j = 2; j < 5; j++) {
				if(toksL[i].length() > j) {
					...
					String suff = toksL[i].substring(toksL[i].length()-j,
							toksL[i].length());
					...
					if(i > 0) {
						...
						String suff_n1 = toksL[i-1].length() > j ? toksL[i-1].substring(toksL[i-1].length()-j) : toks[i-1];
